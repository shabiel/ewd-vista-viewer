...
module.exports = {
  ...
  handlers: {
    ...
    'processSelection' : function(messageObj, session, send, finished) {
      ...
      var componentFileNumber, componentFileName;
      
      if (pComponent === 'securityKey' ) {
        componentFileNumber = 19.1;
        componentFileName = 'SECURITY KEY';
      } else if (pComponent === 'parameterDefinition') {
        componentFileNumber = 8989.51;
        componentFileName = 'PARAMETER DEFINITION';   
      } else if (pComponent === 'option') {
        componentFileNumber = 19;
        componentFileName = 'OPTION'; 
      } else if (pComponent === 'routine') {
        componentFileNumber = 9.8;
        componentFileName = 'ROUTINE';  
      } else {
        return;
      }        
    ...
    'getPatchSummary' : function(messageObj, session, send, finished) {
      ...
      send({
        ...
        params: { 'components': components, 'patch': patch  }
      });
      finished({}); 
      return ;
    },
